...
class DioClient {
  ...
  Future<T> upload<T>({
    ...
    return _send<T>(
      ...
      decoder: decoder,
      body: body,
      // The shared options set no send timeout, which is right for a JSON body but
      // not for a photo pushed over a site connection: without one a stalled upload
      // hangs on a spinner forever instead of failing and letting the user retry.
      sendTimeout: _uploadSendTimeout,
    );
  }

  /// Generous, because it must not abort a slow but progressing upload. A picture is
  /// capped at 10 MB server-side and this client re-encodes to a few hundred
  ...
  Future<T> _send<T>({
    ...
    try {
      ...
      // costs twice: the user is told their connection is down while the backend is
      // demonstrably replying, and session restore takes its offline branch, so a
