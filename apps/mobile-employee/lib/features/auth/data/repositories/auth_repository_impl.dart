...
class AuthRepositoryImpl implements AuthRepository {
  ...
  Future<ApiResult<AppUser>> restoreSession() async {
    ...
    try {
      ...
      // Only the server actually refusing the credential proves the session is dead,
      // so that is the sole case that discards it. Anything else — no connectivity, or
      // a 5xx from a backend whose database is down — says nothing about whether the
      // session is still valid, and signing a technician out mid-shift over a passing
      // outage would force a re-login they may have no signal to complete. Those fall
      // back to the cached user so the app still opens.
