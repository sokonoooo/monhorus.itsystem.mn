...
///
/// Everything beside this file — `service_request_vocabulary.dart`, the `RiskLevel`
/// enum in the Төсөл feature — is a transcription of `packages/shared` compiled into
/// the binary, and therefore frozen at the last store release. That is the right
/// default: a technician standing in a basement with no signal still has to be able
/// to read a chip. But it is only a default. The stage names, the risk-band names and
/// both sets of colours are an administrator's decision, stored under
/// `workflow.request_stages` and `risk.bands`, and `GET /api/v1/vocabulary` is where
/// a client reads them.
///
/// That endpoint exists apart from `GET /settings` for exactly this app's sake: a
/// field technician holds no `settings.view` and never should — the SLA thresholds
/// and the finance keys are none of their business — but the words printed on their
/// own screen are. `/vocabulary` needs authentication and no permission at all.
///
/// **It is a default, not a dependency.** Nothing here is required to render a
/// screen: [ServerVocabulary.empty] is what a failed, refused or not-yet-attempted
/// read leaves behind, every lookup below answers null against it, and every caller
/// falls back to the value it was compiled with. An app that cannot reach the server
/// keeps working in the old words, exactly as it did before this file existed.
///
/// Deliberately free of Flutter, like `service_request_vocabulary.dart` beside it, so
/// the enums can consult it without the domain layer importing the toolkit. The
/// colour NAMES it returns are resolved to a [Tone] by `Tone.named` in the token file,
/// which is the one place that owns the palette.
library;
...
class VocabularyStage {
  ...
  /// the web, a [Tone] triad here — and neither can construct one from an arbitrary
  /// runtime string.
  final String colour;
...
class ServerVocabulary {
  ...
  /// What the app runs on until a read succeeds, and what it goes on running on if
  /// one never does.
  ...
  /// vocabulary this app could not fully parse is still better than none of it, and
...
//
// The things that need these words are `riskSemanticLabel`, `RiskLevel.label`,
// `ServiceRequestStatus.label` and a `CustomPainter` — plain functions and enum
// getters, reached from `Semantics` labels, chip builders and paint calls scattered
// over every screen in the app. None of them holds a `Ref`, and giving them one would
// mean editing every one of their several dozen call sites to thread a provider
// through. That churn is the thing this whole design exists to avoid: the labels had
// to start coming from the server without the screens having to know that they do.
//
...
/// answer lands: every tab is a const widget, and a const widget is canonicalised, so
/// a parent rebuild alone would leave four elements holding the words they were first
/// painted with. It never changes on a failed read, so an offline start costs nothing.
...
/// delete distinctions a technician acts on: the self-progress control offers «Очсон»
/// and «Гүйцэтгэж байна» as two different moves, and both buttons would come out
/// carrying the same word.
///
/// So a stage renames a status only when it covers that status ALONE — then the two
/// are one thing under two names, and the administrator's is the current one. Where a
/// stage groups several statuses its name is still shown, but only where the GROUP is
/// what is being reported: the `stage` the DTO carries on a list row and a detail
/// header, which is a different field and a different question.
...
///
/// Unlike the label this is taken from the group as a whole. A colour is a severity
/// cue rather than a name, so two statuses in one stage sharing it says something
/// true — they are the same step of the job — where sharing a name would not.
...
/// The band keys an administrator has actually configured, in the server's order, or
/// an empty list when no vocabulary has been read.
///
/// The storage vocabulary has eight keys and the shipped ladder uses five; the other
/// three are spares, reserved so the band count can change without a data migration.
/// A legend or a stair drawn from `RiskLevel.values` would print «Түвшин 6» beside
/// «Хэвийн» on every screen in the app, naming three bands nobody configured and no
/// device can be in. This is the list that says which ones are real.
